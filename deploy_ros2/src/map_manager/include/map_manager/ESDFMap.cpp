...
namespace mapManager{
	...
	void ESDFMap::registerESDFPub(){
		this->esdfPub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(this->ns_ + "/esdf", 10);
	}

	void ESDFMap::registerESDFCallback(){
		this->esdfTimer_ = this->create_wall_timer(50ms, std::bind(&ESDFMap::updateESDFCB, this));
		this->esdfPubTimer_ = this->create_wall_timer(50ms, std::bind(&ESDFMap::ESDFPubCB, this));
	}

	void ESDFMap::updateESDFCB(){
		...
		if (this->verbose_){
			cout << this->hint_ << ": ESDF update time: " << (endTime - startTime).seconds() << " s." << endl; 
		}
		this->esdfNeedUpdate_ = false;
	}

	void ESDFMap::ESDFPubCB(){
		this->publishESDF();
	}
